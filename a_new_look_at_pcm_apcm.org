...
\caption{The center-estimation error with respect to degree of uncertainty ($\sigma_v$) under various noise levels ($\alpha$).  The error is computed via $\Sigma_i||\hat{\theta}_i-\theta_i^{True}||$. Note that, the estimated two cluster centers are both considered to be $\hat{\theta}_1$ when the algorithm results in only $1$ cluster. (a) The centers are estimated by UPCM on dataset Fig.\ref{fig1}. In the APCM region, the estimated clusters are almost exactly in the two physical clusters. In the PCM region, the small cluster is "dragged" towards the large cluster. (b) The centers are estimated by APCM on dataset Fig.\ref{fig1}}
