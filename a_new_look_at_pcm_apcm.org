...
\end{equation}
#+END_LaTeX 
respectively, where $d_{ij}=||x_i-\theta_j||$, $\theta_j$s and $u_{ij}^{FCM}$s in \ref{apcm_eta_init} are the final parameter estimates obtained by FCM. $\eta_j$ is updated at each iteration as the /mean absolute deviation/ of the most compatible to cluster $C_j$ data points which form a set $A_j$, i.e., $A_j=\{x_i|u_{ij}=\max_r u_{ir}\}$.
#+BEGIN_LaTeX
\begin{equation}
...
1. present two typical scenarios.
   1. fig1 in apcm. the big cluster is inintialized into two cluster, how to ensure that they do merge?
      must have enough bandwidth to move to the dense region
   2. fig6 in apcm. the two cluster are too close, how to ensure that they don't merge?
      bandwidth shouldn't be too large so that the two clusters don't merge.
      if $\alpha$ is small, the actural bandwidth would be very large, that is why apcm would result in two clusters for fig. (see Fig.7 in \cite{xenaki_novel_2016} for the clustering result of apcm on data set Fig.6)
2. What is the link between PCM and APCM?
