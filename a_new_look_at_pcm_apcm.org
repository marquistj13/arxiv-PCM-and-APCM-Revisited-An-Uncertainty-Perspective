...
\end{equation}
depending on the fuzzy set $V$ whose membership function is $\mu_V(v)$ with $v\in\Omega_V$. The x is called the \emp{primary variable} and $v$ is called the \emp{secondary variable}; the membership function $\mu_{X|V}(x|V)$ characterizes the \emp{primary fuzziness} while the membership function $\mu_V(v)$ characterizes the \emp{secondary fuzziness}.
\end{definition}
#+END_LaTeX
