...
\end{equation}
#+END_LaTeX
where $\sigma_v$ is a given constant which represents the uncertainty of parameter $V$. Then according to Definition \ref{marginal-fs} \eqref{marginal_fs}, the marginal fuzzy set $X$ of $X|V$ with membership function is:
#+BEGIN_LaTeX
\begin{IEEEeqnarray}{ll}
...
\end{equation}
#+END_LaTeX 
Then update of the membership function \eqref{pcm_u_update} is modified according to \eqref{marginal_result} as follows:
#+BEGIN_LaTeX
\begin{equation}
\label{upcm_u_update}
