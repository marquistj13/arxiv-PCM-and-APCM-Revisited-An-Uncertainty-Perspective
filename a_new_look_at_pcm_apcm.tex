...
\item Parameters $\sigma_v$ and $\alpha$ make the clustering process more easy to control. Furthermore, main features of PCM and APCM are unified in this new clustering framework.
\item Experiments show that the bandwidth uncertainty $\sigma_v$ depends on noise level $\alpha$. In fact, the relation between $\sigma_v$ and $\alpha$ is simple: both $\alpha$ and $\sigma_v$ should be large when there are very close clusters in the dataset because we are very uncertain about the estimated bandwidth. This fact reveals that uncertainty of the estimated bandwidth can also be modeled by a type-2 fuzzy set and its uncertainty is caused by $\alpha$. The dependence of $\sigma_v$ on $\alpha$ can be studied in further researches.
\end{enumerate}
